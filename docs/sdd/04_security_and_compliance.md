...
| **THREAT-12** | Default Demo Credential Exploitation & Production Data Breach | Unauthorized account takeover and data breach via known demo accounts (`free@`, `basic@`, `premium@`, `admin.demo@`, `superadmin@dietdost.app`) | Dual compile-time (`#if DEBUG`) and runtime (`IAppEnvironment.AllowsDemoUsers`) isolation. In Release mode or non-Development hosting environments: demo user seeding is suppressed, `LoginCommandHandler` strictly rejects demo credentials (`403 DemoAccessForbidden`), and database startup pro-actively deactivates any pre-existing demo accounts and revokes security stamps. | `DemoUserEnvironmentSecurityTests.*` |
